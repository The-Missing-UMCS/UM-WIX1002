...
public class SubstitutionCipher implements MessageCipher {
    ...
    public String encode(String message) {
        return transform(message, (ch) -> encode(ch,
            ...
            Character.isUpperCase(ch) ? UPPER_A_ASCII : LOWER_A_ASCII));
    }
    ...
    public String decode(String message) {
        return transform(message, (ch) -> decode(ch,
            shiftKey == null ? DEFAULT_SHIFT_KEY : shiftKey,
            Character.isUpperCase(ch) ? UPPER_Z_ASCII : LOWER_Z_ASCII));
    }
     ...
     * <p>
     * The formula {@code (((int) ch + shiftKey - startKey) % NUM_OF_ALPHABET)} calculates the new
     * ASCII value
     * of the character after shifting. The modulo operation ensures the result stays within the
     ...
     * </pre>
     * <p>
     * The subtraction of {@code ((int) ch + shiftKey - startKey) / NUM_OF_ALPHABET} deals with the
     * edge cases.
     *
     * @param ch the character to encode.
     * @param shiftKey the number of positions to shift (can be positive or negative).
     * @param startKey the ASCII value of the start of the alphabet ('A' for uppercase, 'a' for
     * lowercase).
     * @return the encoded character.
     */
    private char encode(char ch, int shiftKey, int startKey) {
        return (char) (((int) ch + shiftKey - startKey) % NUM_OF_ALPHABET + startKey
                       - ((int) ch + shiftKey - startKey) / NUM_OF_ALPHABET);
    }
     ...
     * <p>
     * The {@code endKey} is the ASCII value of the uppercase 'Z' or lowercase 'z', depending on the
     * case of the input character.
     * <p>
     * For example, if {@code ch} is 'D' (ASCII 68) and {@code shiftKey} is 3, the decoded character
     * will be 'A' (ASCII 65).</li>
     * <p>
     * The decoding formula is:
     * <pre>
     * {@code endKey - ((endKey - (int) ch + shiftKey) % NUM_OF_ALPHABET)}
     * </pre>
     * This ensures that after shifting backward by {@code shiftKey}, the result stays within the
     * valid range of the alphabet.
     * <p>
     * Example Calculation:
     * <p>
     ...
     * @param endKey the ASCII value of the end of the alphabet ('Z' for uppercase, 'z' for
     * lowercase).
