...
public class Generator {
    ...
    public static final String COURSE_FILE = chainDir(LAB7, IO_FILES, "coursename.dat");
    public static final String TEXT_FILE = chainDir(LAB7, IO_FILES, "text.txt");
    public static final String ORDER_FILE = chainDir(LAB7, IO_FILES, "order.txt");
    public static final String PRODUCT_FILE = chainDir(LAB7, IO_FILES, "product.txt");
    public static final String PERSON_FILE = chainDir(LAB7, IO_FILES, "person.dat");

    @SneakyThrows
    public static void main(String[] args) {
        // 1. Create the directory if not exists
        String root = chainDir(LAB7, IO_FILES);
        Path path = Paths.get(root);

        if (!Files.exists(path) || !Files.isDirectory(path)) {
            System.out.println(new File(root).mkdir());
        }
    ...
    private static void writeIfNotExists(String filename, String content) {
        generateIfNotExists(filename, () -> {
            ...
            try {
                Files.write(Path.of(filename), lines, StandardOpenOption.CREATE,
    ...
    private static void writeCourseFile() {
        generateIfNotExists(COURSE_FILE, () -> {
                try (
                    var fileOutputStream = new FileOutputStream(COURSE_FILE);
    ...
    record Order(
        ...
        public static Order of(String id, String name, int quantity) {
            return new Order(id, name, quantity);
        }
